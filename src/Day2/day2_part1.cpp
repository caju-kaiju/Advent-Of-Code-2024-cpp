...
int main() {
    ...
    while (std::getline(file, line)) {
        ...
        bool safe = true;
        Direction target_direction = get_direction(levels.at(0), levels.at(1));

        unsigned int previous_num = levels.at(0);
        for (auto num : std::views::all(levels) | std::views::drop(1)) {
